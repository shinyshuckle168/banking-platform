...
- `createdAt` — UTC Instant. System-managed, set at creation
- `updatedAt` — UTC Instant. System-managed, updated on each change
- `deletedAt` — UTC Instant, optional. System-managed. Set when the customer
  record is soft-deleted. Null for all currently active records

Relationships: One customer may own many accounts. Customer records are
managed by Group 1 and consumed by Group 3. The user relationship is managed
externally and not present in the Customer entity.

#### Account

A customer-held banking product. Account type is immutable after creation.

- `accountId` — long, system-assigned unique identifier for the account. Set
  programmatically — not auto-generated by the database. Used as the path
  parameter in all account-level endpoints
...
- `closedAt` — UTC Instant, optional. System-managed. Set when the account
  status changes to CLOSED. Used to enforce the 90-day post-closure access
  window for transaction history retrieval
- `deletedAt` — UTC Instant, optional. System-managed. Set when the account
  record is soft-deleted. Null for all currently active records
- `version` — long. Optimistic locking version counter managed by JPA.
  Read-only externally. Not returned in API responses
- `createdAt` — UTC Instant. System-managed, set at creation, never updated
  thereafter (`updatable = false`)
- `updatedAt` — UTC Instant. System-managed, updated on every write to the
  account record
...
- `transactionId` — string, system-generated unique identifier for the
...
- `direction` — string enum, one of CREDIT, DEBIT, or TRANSFER. Determines the
  direction of the monetary operation
...
- `idempotencyKey` — string, optional. Present only on transactions originating
  from requests that supplied an `Idempotency-Key` header. Used to detect and
...
#### IdempotencyRecord — Stored Entity

The idempotency record is persisted in the `idempotency_record` table. Each
record represents one completed operation associated with one caller.

- `storageKey` — string, primary key, maximum 320 characters. Composite value
  constructed as `callerUserId + ":" + idempotencyKey`. Guarantees global
  uniqueness across callers sharing the same raw `Idempotency-Key` header value
- `idempotencyKey` — string, maximum 255 characters. The raw value supplied
  in the `Idempotency-Key` request header. Retained for secondary lookup
- `callerUserId` — string, maximum 64 characters. The identity of the caller
  who initiated the original request, derived from the authenticated principal
- `operationType` — string, maximum 50 characters. A label identifying the
  category of operation made idempotent, for example `STANDING_ORDER_CREATE`,
  `STANDING_ORDER_EXECUTE`, `TRANSACTION_HISTORY`, or `STATEMENT_REQUEST`
- `responseStatus` — int. The HTTP status code of the original response stored
  so the exact status can be replayed on a duplicate request
- `responseBody` — text. The full response body of the original response stored
  verbatim for replay
- `createdAt` — UTC Instant. System-managed, set at first persistence, never
  updated thereafter. Used by the purge job to identify expired records
...
- The `accountExisted` flag is derived from the account `createdAt` timestamp
  ...
  `UPDATE account SET created_at = '2026-01-01T00:00:00' WHERE account_id = 1`
  *(clarification 2026-04-09)*
...
The following clarifications reflect Account and IdempotencyRecord entity
refactors applied during integration alignment.

### GAP-7 — Account entity: data model alignment

**Spec sections updated**: Shared Definitions → Account domain object;
US-12 Edge Cases (table name in SQL snippet corrected)
**Finding**: Multiple fields were missing from the Account domain object spec
and the table name was inconsistent with the aligned data model.
**Changes applied**:
- `accountId` updated: no longer auto-generated by the database. Set
  programmatically before `persist`. Spec language changed from
  "system-generated" to "system-assigned".
- `closedAt` added: UTC Instant, optional, set on account closure. Used to
  enforce the 90-day post-closure transaction history window.
- `deletedAt` added: UTC Instant, optional, set on soft-delete.
- `version` added: JPA optimistic locking counter, read-only externally.
- `createdAt` updated: stored as UTC Instant with `updatable = false`.
- `updatedAt` updated: stored as UTC Instant, set on every write.
- US-12 Edge Cases SQL snippet corrected from `UPDATE accounts` to
  `UPDATE account` to match the renamed table.

### GAP-8 — IdempotencyRecord entity: rename and schema update

**Spec sections updated**: Shared Definitions → Idempotency section
  (IdempotencyRecord entity sub-section added)
**Finding**: The `IdempotencyRecordEntity` class and `idempotency_records`
table lacked a composite primary key strategy, caller identity, and operation
type, which prevented distinguishing duplicate requests from different callers
using the same raw `Idempotency-Key` header value.
**Changes applied**:
- Class renamed from `IdempotencyRecordEntity` to `IdempotencyRecord`.
- Table renamed from `idempotency_records` to `idempotency_record`.
- New composite primary key field `storageKey` added:
  `storageKey = callerUserId + ":" + idempotencyKey`.
- `idempotencyKey` demoted from `@Id` to a regular non-nullable column.
  Retained for secondary lookup via `findByIdempotencyKey`.
- `callerUserId` added: the authenticated principal identity stored with
  each record to scope idempotency per caller.
- `operationType` added: labels the category of operation, for example
  `STANDING_ORDER_EXECUTE` or `TRANSACTION_HISTORY`.
- `statusCode: Integer` renamed to `responseStatus: int` (primitive) with
  column `response_status`.
- `responseBody` annotation changed from `columnDefinition = "TEXT"` to
  `@Lob` for portable LOB mapping.
- `createdAt` changed from `LocalDateTime` to UTC Instant with
  `updatable = false`. Setter removed — field is immutable after persistence.
- IdempotencyRecord sub-section added to the Idempotency shared definition
  documenting all fields and the composite key construction rule.
