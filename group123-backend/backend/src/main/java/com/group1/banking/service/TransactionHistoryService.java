...
public class TransactionHistoryService {
    ...
    public TransactionHistoryResponse getHistory(long accountId, LocalDate startDate,
        ...
        Instant effectiveEnd = (endDate != null)
            ? endDate.plusDays(1).atStartOfDay(zone).minusNanos(1).toInstant()
                : now;
        ...
        LocalDate startLocalDate = effectiveStart.atZone(zone).toLocalDate();
        LocalDate endLocalDate = effectiveEnd.atZone(zone).toLocalDate();
        ...
        return exportCacheRepository.findByAccountIdAndParamHash(accountId, paramHash)
                ...
                .orElseGet(() -> {
                    ...
                    byte[] pdfBytes = pdfStatementService.buildPdf(
                            accountId,
                            effectiveStart.atZone(zone).toLocalDate(),
                            effectiveEnd.atZone(zone).toLocalDate(),
                            txns);
