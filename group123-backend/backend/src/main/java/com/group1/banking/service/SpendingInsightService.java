...
public class SpendingInsightService {
    ...
    private static final List<String> VALID_CATEGORIES = List.of(
            ...
            "Shopping", "Utilities", "Health", "Income", "Other");
    private static final String UNCATEGORISED_LABEL = "Uncategorised";
    ...
    public RecategoriseResponse recategorise(long accountId, String transactionId,
        ...
        // Validate category (allow explicit uncategorised to clear category)
        if (!"Uncategorised".equalsIgnoreCase(normalizedCategory)
            && !VALID_CATEGORIES.contains(normalizedCategory)) {
            throw new SemanticValidationException(
                "Invalid category. Must be one of: " + VALID_CATEGORIES + " or Uncategorised",
        ...
        String updatedCategory = "Uncategorised".equalsIgnoreCase(normalizedCategory) ? null : normalizedCategory;
        ...
        response.setUpdatedCategory(updatedCategory);
    ...
    private List<CategoryBreakdownItem> buildBreakdown(List<Transaction> eligible,
        ...
        totals.put(UNCATEGORISED_LABEL, BigDecimal.ZERO);
        counts.put(UNCATEGORISED_LABEL, 0);
        ...
        for (Transaction t : eligible) {
            ...
            if (cat != null && totals.containsKey(cat)) {
                ...
                totals.put(UNCATEGORISED_LABEL, totals.get(UNCATEGORISED_LABEL).add(t.getAmount()));
                counts.put(UNCATEGORISED_LABEL, counts.get(UNCATEGORISED_LABEL) + 1);
            }
        }

        List<String> categories = new ArrayList<>(VALID_CATEGORIES);
        categories.add(UNCATEGORISED_LABEL);
